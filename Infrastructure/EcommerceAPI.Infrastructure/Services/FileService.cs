...
namespace EcommerceAPI.Infrastructure.Services
{
    public class FileService : IFileService
    {
        ...
        public async Task<List<(string fileName, string path)>> UploadAsync(string path, IFormFileCollection files)
        {
            ...
            {
                string renamedFile = await FileRenameAsync(uploadPath, file.FileName);

                bool result = await CopyFileAsync($"{uploadPath}\\{renamedFile}", file);
                datas.Add((renamedFile, $"{uploadPath}\\{renamedFile}"));
                results.Add(result);
            }
